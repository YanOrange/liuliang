...
class CultivateCourse extends BaseValidate
{
    //数组顺序就是检测的顺序
    protected $rule = [
        'merchant_id'   => 'require',
    ...
    /*protected function checkCountCourse($merchant_id, $rule, $data) {
        ...
        $courseId = CourseModel::where('merchant_id', $merchant_id)->where('course_type', 2)->value('id');
        if (!isset($data['id'])) {
            if ($courseId) {
    ...
    protected $scene = [
        'add' => ['merchant_id','title','channel_ids','part_class_ids','content','status','virtual_apply_nums','video_cover_image','entry_fee','original_price','sort','btn_desc'],
        'edit' => ['merchant_id','title','channel_ids','part_class_ids','content','status','virtual_apply_nums','video_cover_image','entry_fee','original_price','sort','btn_desc'],
    ];
}