...
//课程报名客服分配
class CustomerServiceOverdue
{
    const MERCHANT_LIST_ID = [142,245,246,252,258,259];
    public function getCustomerServiceId($merchantId = 0)
    {
        /*$customerList = $this->getCustomerList($merchantId);
        var_dump($customerList);die;*/
        $serviceId = 0;
        $redisKey = env('redis.yuanhua_customer_redis_key') . '_app_class_id_9';
        $redis = get_redis();
        if (!$redis->exists($redisKey)) {  //初始化赋值
            ...
            foreach ($list as $key => $val) {
                ...
                if (in_array($arr['merchant_id'], self::MERCHANT_LIST_ID) && $arr['status'] && $arr['period_thread_num'] > 0) {
                    //if ($arr['residue_thread_num'] > 0 && $arr['status'] && $arr['period_thread_num'] > 0) {
                    $data[] = ['id' => $key, 'weight' => 1];
                    $sumPeriodThreadNum += $arr['period_thread_num'];
                }
            }
            // var_dump($data);
            if ($sumPeriodThreadNum <= 0 || empty($data)) { //当所有的落地页的曝光都为0时,重新初始化
                $serviceId = $this->initCustomerServiceData($redis, $redisKey, $merchantId);
            } else {
                if (!empty($data)) {
    ...
    {
        return  Merchant::whereIn('id', self::MERCHANT_LIST_ID)->column('id');
    }
    //初始化客服分配的线索数量
