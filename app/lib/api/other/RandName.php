...
class RandName
{
      ...
      */
      ...
      public  function getRandPhone()
      {
            ...
            return  substr_replace($arr[array_rand($arr)].mt_rand(1000,9999).''.mt_rand(1000,9999),'****',3,4) ;
            
      }
      //获取跑马灯数据
      public function getHorseRaceLampData($nums = 10, $isPhone = true, $isName = false, $isTime = true)
      {
          $data = [];
          for ($i = 0; $i < $nums; $i++) {
              $arr = [];
              if ($isPhone) {
                  $arr['phone'] = $this->getRandPhone();
              }
              if ($isName) {
                  $arr['username'] = $this->getName(0);
              }
              if ($isTime) {
                  $arr['time'] = rand(1,30) . '分钟前';
              }
              $data[] = $arr;
          }
          return $data;
