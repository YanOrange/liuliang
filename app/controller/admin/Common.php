...
class Common extends Backend
{
    ...
    public function upload()
    {
        try {
            ...
            $uploadType = $this->request->param('upload_type');
            //var_dump($uploadType);die;
            if ($uploadType != 'local-url') {
                $uploadType = $uploadType == 'ali-oss-url' ? $uploadType : env('upload.uploadtype', 'local');

            }
            if (!in_array($uploadType, ['local', 'ali-oss', 'local-url','ali-oss-url','qiniu-kodo'])) {
            ...
            //上传至阿里云
            if ($uploadType == 'ali-oss' || $uploadType == 'ali-oss-url') {
            ...
            //本地上传
            if ($uploadType == 'local' || $uploadType == 'local-url') {
            ...
            return $this->success('上传成功', [
                'id' => $uploadType == 'ali-oss-url' || $uploadType == 'local-url' ? $inputValue : $fileId,
                'path' => $inputValue,
                'name' => $file->getOriginalName(),
            ]);
        } catch (\Exception $e) {
    ...
    public function aliSts()
    {
        ...
        $stsConf = [
            'accessKeyID' => env('aliosssts.ALIYUN_OSSSTS_ACCESSKEYID'),
            'accessKeySecret' => env('aliosssts.ALIYUN_OSSSTS_ACCESSKEYSECRET'),
            'ARN' => env('aliosssts.ALIYUN_OSSSTS_ARN'),
            'endpoint' => env('aliosssts.ALIYUN_OSSSTS_ENDPOINT'),
            'domain' => env('aliosssts.ALIYUN_OSSSTS_DOMAIN'),
            'bucket' => env('aliosssts.ALIYUN_OSSSTS_BUCKET'),
        ];
