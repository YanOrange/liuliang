...
use app\validate\admin\biclassqrcodenumber\BiClassQrcodeNumber as BiClassQrcodeNumberValidate;
 ...
 * 后台应用控制器
 */
class BiClassQrcodeNumber extends Backend
{
    protected $model;//当前模型对象

    protected function _initialize()
    {
        $this->model = new \app\model\admin\BiClassQrcodeNumber();
    }
    //查看
    public function index()
    {
        ...
        $data = $this->model->where($where)->with(['admin'])->order($order);
    ...
    public function add()
    {
        ...
        $validate = new BiClassQrcodeNumberValidate();
        if (!$validate->scene('add')->check($post)) return $this->error($validate->getError());
        $timer = strtotime($post['thedate']);
