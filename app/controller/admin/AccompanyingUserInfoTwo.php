...
use app\model\admin\ProvinceCity;

/**
 * 后台应用分类控制器
 */
class AccompanyingUserInfoTwo extends Backend
{
    protected $model;//当前模型对象
    protected $noNeedAuth = ['provinceCityList','hospitalList','hospitalSkuList'];

    protected function _initialize()
    {
        $this->model = new \app\model\admin\AccompanyingUserInfo();
    }
    ...
    public function index()
    {
        ...
        $data = $this->model->with(['hospital'])->where($where)->where('type',2)->order($order);
    ...
    public function provinceCityList()
    {
        ...
        if(!empty($cityId)){
            $data = ProvinceCity::where('father_value',$cityId)->field('value as id,name')->order('id asc');
        }else{
            $data = ProvinceCity::where('level',2)->field('value as id,name')->order('id asc');
        }
        $allData = $this->request->param('all_data');
        if ($allData) {
            $data = $data->select();
    ...
    {
        $data = AccompanyingHospital::order('id asc');
        $allData = $this->request->param('all_data');
        if ($allData) {
            $data = $data->select();
        } else {
    ...
    //陪诊医院SKU
    public function hospitalSkuList()
    {
        $data = AccompanyingSku::whereIn('id',[13,14])->order('id asc');
        $allData = $this->request->param('all_data');
        if ($allData) {
            $data = $data->select();
        } else {
...
//        if (!$validate->scene('add')->check($post)) return $this->error($validate->getError());
        ...
        try {
            if(!empty($post['phone'])){
                $wechatUser = $this->model->where('phone',$post['phone'])->find();
                if(!empty($wechatUser)){
