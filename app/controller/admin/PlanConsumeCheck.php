...
use think\facade\Db;
use app\validate\admin\planconsumecheck\PlanConsumeCheck as PlanConsumeCheckValidate;
use app\model\api\PlanDetailData;
use app\model\api\VivoPlanDetailData;
use app\model\admin\DouyinPlanDetailData;
use app\model\admin\Channel;

/**
 * 后台应用分类控制器
 */
class PlanConsumeCheck extends Backend
{
    protected $model;//当前模型对象
    protected function _initialize()
    {
        $this->model = new \app\model\admin\PlanConsumeCheck();
    }
    //查看
    ...
    public function add()
    {
        ...
        $validate = new PlanConsumeCheckValidate();
        if (!$validate->scene('add')->check($post)) return $this->error($validate->getError());
        $where[] = ['channel','=',$post['channel']];
        $totalConsume = 0;
        if(strstr($post['channel'], 'oppo') !== false){
            $where[] = ['plantime','between',[strtotime($post['plantime']),strtotime($post['plantime'].' 23:59:59')]];
            $totalConsume = PlanDetailData::where($where)->sum('consume_amount');
        }
        if(strstr($post['channel'], 'vivo') !== false){
            $where[] = ['report_time','between',[strtotime($post['plantime']),strtotime($post['plantime'].' 23:59:59')]];
            $totalConsume = VivoPlanDetailData::where($where)->sum('spent');
        }
        if(strstr($post['channel'], 'douyin') !== false){
            $where[] = ['stat_datetime',date('Y-m-d',strtotime($post['plantime']))];
            $totalConsume = DouyinPlanDetailData::where($where)->sum('cost');
        }
    ...
    public function upload()
    {
        ...
        foreach ($sheet as $k => $v) {
            ...
            $channelRes = $this->model->where('plantime',$startTime)
                ->where('channel',$v[0])
                ->find();
            if ($channelRes) {
            ...
            if(strstr($v[0], 'vivo') !== false){
                $totalConsume = VivoPlanDetailData::where('channel',$v[0])
                    ->whereBetween('report_time',[strtotime($startTime),strtotime($startTime.' 23:59:59')])
                    ->sum('spent');
            }
            if(strstr($v[0], 'douyin') !== false){
                $totalConsume = DouyinPlanDetailData::where('channel',$v[0])
                    ->where('stat_datetime',date('Y-m-d',strtotime($startTime)))
                    ->sum('cost');
            }
            $post['total_consume'] = $totalConsume;
            $arr[] = [
