...
use app\validate\admin\advertising\Advertising as AdvertisingValidate;
...
class Advertising extends Backend
{
    ...
    protected $noNeedAuth = ['info', 'getChannelTree'];

    protected function _initialize()
    {
        $this->model = new \app\model\admin\Advertising();
    }
    ...
    public function index()
    {
        ...
        $data = $this->model->with(['page'])->where($where)->where($whereCon)->order($order);
    ...
    public function getChannelTree()
    {
        ...
        $channelIds = [];
        if ($id) {
            $find = Db::name($table)->field('id, channel_ids')->where('id', $id)->find();
            $channelIds = explode(',', $find['channel_ids']);
        }

        $class = AppClass::field('id, app_class_name')->select()->toArray();
        $channel = Db::name('channel')
            ->alias('c')
            ->field('c.id, c.app_id, a.app_class_id, a.app_name, c.channel_name')
        ...
        foreach ($channel as $item) {
            ...
            $channelArr[$item['app_class_id']][] = [
                'id' => '|' . $item['app_class_id'] . '_' . $item['id'],
                'name' => $item['channel_name'],
                'title' => $item['channel_name'] . ' <> ' . $item['app_name'],
    ...
    public function add()
    {
        $allowField = ['is_many_organization', 'merchant_id', 'channel_ids', 'page_id', 'title', 'image', 'sort', 'open_mode',
            'jump_mode','status'
        ];
...
//        if($post['jump_mode'] == 1){
//            if(!$post['module_id'] || (!$post['course_id'] && !$post['article_id'])){
//                return $this->error('跳转参数错误');
//            }
//            $post['course_id'] = !empty($post['course_id']) ? $post['course_id'] : $post['article_id'];
...
//        }
    ...
    {
        // 渠道ID @chenlele 1028 start
        $allowField = ['id', 'is_many_organization', 'merchant_id', 'channel_ids', 'page_id', 'title', 'image', 'sort', 'open_mode',
            'jump_mode', 'status'];
        $post = CommonFun::filterPostData(Request::only($allowField));
