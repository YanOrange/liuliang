 ...
 * 后台应用分类控制器
 */
class CustomerPaiban extends Backend
{
    ...
    public function index()
    {
        ...
        }])->where($where)->where('data_type',1)->order($order);
    ...
    public function add()
    {
        ...
        if (!$validate->scene('addDemand')->check($post)) return $this->error($validate->getError());
        $post['data_type'] = 1;
    ...
    public function edit()
    {
        $post     = CommonFun::filterPostData($this->request->post());
        $validate = new CustomerPaibanValidate();
        if (!$validate->scene('editDemand')->check($post)) return $this->error($validate->getError());
        Db::startTrans();
        try {
