...
use app\validate\admin\single\CourseTwo as CourseTwoValidate;
use laytp\controller\Backend;
use think\facade\Db;
use app\model\admin\PartCourseTag;
 ...
 */
class CourseTwo extends Backend {
    ...
    private function buildSearch($isDelete = false) {
        ...
        if (isset($merchant_id) && !empty($merchant_id)) {
            $courseModel = $courseModel->whereFindInSet('merchant_ids', $merchant_id);
        }
        ...
        if (isset($tag_id) && is_numeric($tag_id)) {
            $courseModel = $courseModel->where('tag_id', '=', $tag_id);
        }
    ...
    public function tagList()
    {
        ...
        $whereCon = [];
        if(!empty($type)){
            $whereCon[] = ['tag_type','=',$type];
        }
        $data = PartCourseTag::where($whereCon)->order('id desc');
        $allData = $this->request->param('all_data');
        if ($allData) {
            $data = $data->select();
        } else {
    ...
    public function add() {
        ...
        $validate = new CourseTwoValidate();
