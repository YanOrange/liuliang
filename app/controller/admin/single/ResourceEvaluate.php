...
use app\validate\admin\single\Evaluate as EvaluateValidate;
use laytp\controller\Backend;
use think\facade\Db;

/**
 * 课程控制器
 * Class ResourceEvaluate
 * @package app\controller\admin\single
 */
class ResourceEvaluate extends Backend {
    protected $model;//当前模型对象

    protected function _initialize() {
        $this->model = new \app\model\admin\single\Evaluate();
    }
    ...
    public function index() {
        ...
        $whereCon = [['be_evaluated_type','=',2]];
        $beEvaluatedId= $this->request->param('be_evaluated_id', 0);
        if ($beEvaluatedId) {
            $whereCon[] = ['be_evaluated_id', '=', $beEvaluatedId];
        }
    ...
    private function buildSearch($isDelete = false) {
        ...
        if ($isDelete) {
            $evaluateModel = $this->model->onlyTrashed();
        } else {
            $evaluateModel = $this->model;
        }
        $evaluateModel = $evaluateModel->where('be_evaluated_type', '=', 2);
        if (isset($be_evaluated_id) && is_numeric($be_evaluated_id)) {
            $evaluateModel = $evaluateModel->where('be_evaluated_id', '=', $be_evaluated_id);
        }
        if (isset($is_real) && is_numeric($is_real)) {
            $evaluateModel = $evaluateModel->where('is_real', '=', $is_real);
        }
        if (isset($score) && is_numeric($score)) {
            $evaluateModel = $evaluateModel->where('score', '=', $score);
        }
        if (isset($status) && is_numeric($status)) {
            $evaluateModel = $evaluateModel->where('status', '=', $status);
        }
        if (isset($create_time) && !empty($create_time)) {
            list($startTime, $endTime) = explode(' - ', $create_time);
            $evaluateModel = $evaluateModel->where('create_time', 'between', strtotime($startTime) . ',' . strtotime($endTime));
        }
