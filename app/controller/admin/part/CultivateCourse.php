...
use app\validate\admin\part\CultivateCourse as CultivateCourseValidate;
 ...
 * 后台培训课程控制器
 */
class CultivateCourse extends Backend
{
    ...
    public function index()
    {
        ...
        $data = $this->buildSearch()->where('course_type', 2)->with(['merchant'])->order($order);
    ...
    public function add()
    {
        $allowField = ['merchant_id','channel_ids','app_class_id','status','title','part_class_ids','tag_ids','entry_fee',
            'original_price','video_cover_image','video_url','virtual_apply_nums','sort','share_desc','btn_desc','course_ids',
            'is_force_wx','gather_form_json','video_resource_ids','content','part_course_bottom_desc'
        ];
        ...
        $validate = new CultivateCourseValidate();
        if (!$validate->scene('add')->check($post)) return $this->error($validate->getError());
        $gatherFormNameArr = $post['gather_form_json']['form_name'];
        $gatherFormHrefArr = $post['gather_form_json']['form_href'];
        $gatherFormArr = [];
        for($i = 0; $i < count($gatherFormNameArr); $i++) {
            $gatherFormArr[] = ['form_name' => $gatherFormNameArr[$i], 'form_href' => $gatherFormHrefArr[$i]];
        }
        $post['gather_form_json'] = json_encode($gatherFormArr, JSON_UNESCAPED_UNICODE);
    ...
    public function info()
    {
        ...
        $info['gather_form_json'] = json_decode($info['gather_form_json'], true);
    ...
    {
        $allowField = ['id','merchant_id','channel_ids','app_class_id','status','title','part_class_ids','tag_ids','entry_fee',
            'original_price','video_cover_image','video_url','virtual_apply_nums','sort','share_desc','btn_desc','course_ids',
            'is_force_wx','gather_form_json','video_resource_ids','content','part_course_bottom_desc'
        ];
        $post     = $this->request->post(Request::only($allowField));
