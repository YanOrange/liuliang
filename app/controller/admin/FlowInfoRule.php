...
use app\validate\admin\for_flow\FlowInfoRule as FlowInfoRuleValidate;
 ...
 * 后台应用分类控制器
 */
class FlowInfoRule extends Backend
{
    protected $model;//当前模型对象
    protected $noNeedAuth = ['channelList'];

    protected function _initialize()
    {
        $this->model = new \app\model\admin\FlowInfoRule();
    }
    //查看
    ...
    {
        $data = Channel::where('channel_name','like','yqh5%')->order('id asc');
        $allData = $this->request->param('all_data');
        if ($allData) {
            $data = $data->select();
        } else {
    ...
    public function add()
    {
        ...
        $validate = new FlowInfoRuleValidate();
