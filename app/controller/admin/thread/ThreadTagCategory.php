...
use think\facade\Db;
use app\validate\admin\thread\ThreadTagCategory as ThreadTagCategoryValidate;
 ...
 */
class ThreadTagCategory extends Backend {
    protected $model;//当前模型对象

    protected function _initialize() {
        $this->model = new \app\model\admin\thread\ThreadTagCategory();
    }
    ...
    private function buildSearch($isDelete = false) {
        ...
        if ($isDelete) {
            $tagCategoryModel = $this->model->onlyTrashed();
        } else {
            $tagCategoryModel = $this->model;
        }

        if (isset($title) && !empty($title)) {
            $tagCategoryModel = $tagCategoryModel->where('title', 'like', "%{$title}%");
        }
        if (isset($create_time) && !empty($create_time)) {
            list($startTime, $endTime) = explode(' - ', $create_time);
            $tagCategoryModel = $tagCategoryModel->where('create_time', 'between', strtotime($startTime) . ',' . strtotime($endTime));
        }
    ...
    public function add() {
        ...
        $validate = new ThreadTagCategoryValidate();
