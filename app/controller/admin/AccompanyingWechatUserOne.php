 ...
 * 后台应用分类控制器
 */
class AccompanyingWechatUserOne extends Backend
{
    protected $model;//当前模型对象
    protected $noNeedAuth = ['provinceCityList'];

    protected function _initialize()
    {
        $this->model = new \app\model\admin\AccompanyingWechatUser();
    }
    ...
    public function provinceCityList()
    {
        ...
        if(!empty($provinceId)){
            $data = ProvinceCity::where('father_value',$provinceId)->field('value as id,name')->order('id asc');
        }else{
            $data = ProvinceCity::where('level',1)->field('value as id,name')->order('id asc');
        }
        $allData = $this->request->param('all_data');
        if ($allData) {
            $data = $data->select();
...
//        if (!$validate->scene('add')->check($post)) return $this->error($validate->getError());
        ...
        try {
            ...
            if(!empty($post['wechat_account'])){
                $wechatUser = $this->model->where('wechat_account',$post['wechat_account'])->find();
                if(!empty($wechatUser)){
...
//        if (!$validate->scene('edit')->check($post)) return $this->error($validate->getError());
        ...
        try {
            ...
            if(strpos($post['wechat_account'], '****')){
                unset($post['wechat_account']);
            }
