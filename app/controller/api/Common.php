...
{
    public $noNeedLogin = ['sendEmailCode','checkEmailCode','sendMobileCode', 'config'];

    public $noNeedCheckSign = ['config'];
...
({
    ...
    public function config()
    {
        ...
        if ($channel) {     // 是否跳转微信
            ...
            $pageShow = ($channelInfo && $channelInfo->is_jump_miniprogram) ? true : false;
        }

        $statementShow = true;
        if ($channelId) {   // B站不展示协议信息
            $channelInfo = Channel::where('id', $channelId)->find();
            $statementShow = ($channelInfo && $channelInfo->is_show_personal_statement) ? true : false;
        }


        return $this->success('发送成功', ['page_show' => $pageShow, 'statement_show' => $statementShow]);
    }
}