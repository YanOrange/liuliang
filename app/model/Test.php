...
class Test extends BaseModel
{
    ...
    public function selAdminUser(){
        return $this->belongsTo('app\model\admin\User','sel_admin_id','id');
    }
	...
	{
		return (isset($data['videos']) && $data['videos']) ? UploadDomain::multiJoin($data['videos']) : '';
	}

	public function getMusicsFileAttr($value, $data)
	{
		return (isset($data['musics']) && $data['musics']) ? UploadDomain::multiJoin($data['musics']) : '';
	}

	public function getFilesFileAttr($value, $data)
	{
		return (isset($data['files']) && $data['files']) ? UploadDomain::multiJoin($data['files']) : '';
	}
