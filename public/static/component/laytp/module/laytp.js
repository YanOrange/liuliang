    ...
    laytp.tableFormatter = {
            ...
            if(value || value === 0){
                for (key in json['value']) {
                    if (value.toString() === json['value'][key].toString()) {
                        if(isTreeTable){
                            return '<span  style="color:' + laytp.tableFormatter.custom[customIndex] + ';font-size:14px;">' + json['text'][key] + '</span>';
                        }else{
        ...
        video: function (value) {
            ...
            if (value) {
                ...
                let valueArr = value.split(','), key;
                for (key in valueArr) {
                    html += '<a href="javascript:void(0);" onclick="layui.laytp.tableFormatter.showVideo(\'' + valueArr[key] + '\')" class=\"layui-link\">查看视频</a> ';
                    i++;
                }
            }
            return html;
