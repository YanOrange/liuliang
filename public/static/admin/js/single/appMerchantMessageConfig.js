layui.use(["laytp"], function () {
    const funController = {};
    //静态页面地址前缀
    window.htmlPrefix = facade.compatibleHtmlPath("/admin/single/app_merchant_message_config/");
    //后端接口地址前缀
    window.apiPrefix = facade.compatibleApiRoute("/admin.single.app_merchant_message_config/");
    ...
    funController.tableRender = function (where, page) {
        layui.table.render({
            elem: "#laytp-table"
            , id: "laytp-table"
            , url: facade.url("/admin.single.app_merchant_message_config/index")
            ...
            , cols: [[
                ...
                , {field: "num", title: "消息数量", align: "center", width: 180}
            ...
            if (defaultTableTool.indexOf(obj.event) !== -1) {
                ...
                switch (obj.event) {
                    ...
                    case "showMessageList":
                        facade.popupDiv({
                            title: "消息列表"
                            , path: "/admin/single/app_merchant_message/index.html?app_message_id=" + obj.data.id
                        });
                        break;
                    case "addMessage":
                        let options = {};
                        options.title = "新增消息";
                        options.path = facade.compatibleHtmlPath(window.htmlPrefix) + "add_message.html?id="+obj.data.id;
