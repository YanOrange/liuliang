layui.use(["laytp"], function () {
    ...
    funController.tableRender = function (where, page) {
        layui.table.render({
            elem: "#laytp-table"
            , id: "laytp-table"
            , url: facade.url("/admin.ThreadLaborArbitration/index")
            ...
            , cols: [[
                ...
                , { field: "situation_describe", title: "跟进情况描述", align: "center", width: 120, templet:function(d){
                    return laytpForm.tableForm.editInput('situation_describe',d,'/admin.ThreadLaborArbitration/setFind');
                }} 
                , { field: "signed_amount ", title: "签单金额", align: "center", width: 120, templet:function(d){
                    return laytpForm.tableForm.editInput('signed_amount',d,'/admin.ThreadLaborArbitration/setFind');
                } }
                , { field: "actual_received_amount", title: "实收金额", align: "center", width: 120, templet:function(d){
                    return laytpForm.tableForm.editInput('actual_received_amount',d,'/admin.ThreadLaborArbitration/setFind');
                } }
        ...
        layui.table.on("toolbar(laytp-table)", function (obj) {
            ...
            if (defaultTableToolbar.indexOf(obj.event) !== -1) {
                ...
                switch (obj.event) {
                    case "exportThread":
                        window.location.href = '/admin.thread_labor_arbitration/exportThread';
                        break;
                }
            }
        });
            ...
            if (defaultTableTool.indexOf(obj.event) !== -1) {
                ...
                switch (obj.event) {
                    ...
                    case "showDetail":
                        facade.popupDiv({
                            title: "线索详情"
                            , path: "/admin/thread_labor_arbitration/thread_detail.html?&id=" + obj.data.id
                        });
                        break;
                }
            }
        ...
        layui.form.on('select(stateSelect)', function (data) {//获取当前行tr对象
            ...
            facade.ajax({
                route: '/admin.ThreadLaborArbitration/setThreadLaborArbitationStatus',
                data: datas,
            }).done(function (res) {
                if (res.code === 0) {
                    location.reload();
        ...
        layui.form.on('select(stateSelect1)', function (data) {//获取当前行tr对象
            ...
            facade.ajax({
                route: '/admin.ThreadLaborArbitration/setThreadLawyerSituation',
                data: datas,
            }).done(function (res) {
                if (res.code === 0) {
                    location.reload();
