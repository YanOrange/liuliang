layui.use(["laytp"], function () {
    const funController = {};
    //静态页面地址前缀
    window.htmlPrefix = facade.compatibleHtmlPath("/admin/point/show_advertising/");
    //后端接口地址前缀
    window.apiPrefix  = facade.compatibleApiRoute("/admin.show_advertising/");
    ...
    funController.tableRender = function (where, page) {
        layui.table.render({
            ...
            , cols: [[
                ...
                , {field: "advertising_id", title: "广告名称", align: "center",width: 150,templet:'<div>{{# if(d.advertising){ }}{{d.advertising.title}}{{# }else{ }}-{{# } }}</div>'}
                , {field: "page_id", title: "页面名称", align: "center",width: 150,templet:'<div>{{# if(d.page){ }}{{d.page.title}}{{# }else{ }}-{{# } }}</div>'}
                , {field: "advertising_id", title: "广告状态", align: "center", width: 150, templet: function (d) {
                        return laytp.tableFormatter.status('advertising',d.advertising.status,{"value":["0","1"],"text":["显示","不显示"]}, true);
                    }}
