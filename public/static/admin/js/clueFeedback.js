layui.use(["laytp"], function () {
    const funController = {};
    //静态页面地址前缀
    window.htmlPrefix = facade.compatibleHtmlPath("/admin/clue_feedback/");
    //后端接口地址前缀
    window.apiPrefix  = facade.compatibleApiRoute("/admin.clue_feedback/");
    ...
    funController.tableRender = function (where, page) {
        layui.table.render({
            elem: "#laytp-table"
            , id: "laytp-table"
            , url: facade.url("/admin.clue_feedback/index")
            ...
            , done: function(res){
                if(res.data[0].feedback_status == 10){
                    $("[lay-event='info']").css('display','none');
                    $("[lay-event='edit']").css('display','block');
                }
                if(res.data[0].feedback_status == 11){
            ...
            if (defaultTableTool.indexOf(obj.event) !== -1) {
                ...
                //自定义按钮点击事件
                    case "info":
                        facade.popupDiv({
                            title: "查看原因"
                            , path: "/admin/clue_feedback/info.html?id=" + obj.data.id
                        });
                        break;
                }
            }
