layui.use(["laytp"], function () {
    const funController = {};
    //静态页面地址前缀
    window.htmlPrefix = facade.compatibleHtmlPath("/admin/wx_user_thread_jyd/");
    //后端接口地址前缀
    window.apiPrefix  = facade.compatibleApiRoute("/admin.wx_user_thread_jyd/");
    ...
    funController.tableRender = function (where, page) {
        layui.table.render({
            elem: "#laytp-table"
            , id: "laytp-table"
            , url: facade.url("/admin.wx_user_thread_jyd/index")
        ...
        layui.table.on("toolbar(laytp-table)", function (obj) {
            ...
            if (defaultTableToolbar.indexOf(obj.event) !== -1) {
                ...
                switch(obj.event) {
                    case "exportThread":
                        window.location.href = '/admin.wx_user_thread_jyd/exportThread';
                        break;
                }
            }
        });
        ...
        layui.table.on('edit(laytp-table)', function (obj) {
            ...
            facade.ajax({
                route:'/admin.wx_user_thread_jyd/setWxNickname',
