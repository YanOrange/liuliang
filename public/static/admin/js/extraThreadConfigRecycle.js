layui.use(['laytp'], function () {
    ...
    //静态页面地址前缀
    window.htmlPrefix = facade.compatibleHtmlPath("/admin/extra_thread_config/");
    //后端接口地址前缀
    window.apiPrefix  = facade.compatibleApiRoute("/admin.extra_thread_config/");
    ...
    funRecycleController.tableRender = function (where, page) {
        layui.table.render({
            ...
            , url: facade.url("/admin.extra_thread_config/recycle")
            ...
            , cols: [[ //表头
                ...
                , {field: "account_id", title: "标识",  align: "center"}
                , {field: "channel_id", title: "关联渠道", align: "center",templet:'<div>{{# if(d.channel){ }}{{d.channel.channel_name}}{{# }else{ }}-{{# } }}</div>'}
                , {field:'source',title:'来源',align:'center',templet:function(d){
                        return laytp.tableFormatter.status('source',d.source,{"value":["1","2","3"],"text":["青团","兼客","淘米乐"]}, true);
                    }}
