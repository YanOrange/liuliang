...
class Log extends Manager implements LoggerInterface
{
    ...
    public function record($msg, string $type = 'info', array $context = [], bool $lazy = true)
    {
        ...
        // @date 2022-10-10 错误日志预警消息推送
        $isCtr = stripos($msg, '控制器不存在');
        if ($type == 'error' && !$isCtr) {
            $cur = date('Y-m-d H:i:s', time());
            $msg = "<font color='gray'>[ {$cur} ]</font> <font color='warning'> [ Server:" .  env('domain.ip') . " <> Client: " . $_SERVER['REMOTE_ADDR'] . " ] </font> $msg" . json_encode($_REQUEST);

            Robot::robotMsg($msg, env('WXROOT.ERRORURL'));
        }
